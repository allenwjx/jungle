...
import ${tableConfig.basepackage}.dataobject.${tableConfig.className}${deployModule}DO;
import ${tableConfig.basepackage}.daointerface.${tableConfig.className}${deployModule}DAO;
...
<#macro generateOperationMethodBody sql>
	...
	<#if sql.selectSql>
		...
		return PageQueryUtils.pageQuery(getSqlMapClientTemplate(),"${ibatisNamespace}${sql.operation}",${paramName});
			<#else>
		return PageQueryUtils.pageQuery(getSqlMapClientTemplate(),"${ibatisNamespace}${sql.operation}",${paramName},pageNum,pageSize);
			</#if>
		<#elseif sql.multiplicity = 'one'>
			<#local resultClassName><@generateResultClassName sql '${deployModule}DO'/></#local>
		...
		if(returnObject == null)
			...
			<#else>
		return (<@generateResultClassName sql '${deployModule}DO'/>)getSqlMapClientTemplate().queryForObject("${ibatisNamespace}${sql.operation}",${paramName});
			</#if>
		<#else>
		return (<@generateResultClassName sql '${deployModule}DO'/>)getSqlMapClientTemplate().queryForList("${ibatisNamespace}${sql.operation}",${paramName});
		</#if>
	</#if>
	<#if sql.deleteSql>
